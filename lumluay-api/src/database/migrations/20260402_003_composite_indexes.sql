...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_orders_tenant_table_status
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_orders_tenant_receipt
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_kitchen_tickets_tenant_status_station
  ON kitchen_tickets (tenant_id, status, station)
  WHERE status NOT IN ('served', 'cancelled');

-- Ticket lookup by order_item (join path from orders → kitchen)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_kitchen_tickets_order_item
  ON kitchen_tickets (order_item_id);

-- ─── Products ─────────────────────────────────────────────────────────────────
-- Active products per tenant + category (menu listing)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_products_tenant_active_category
...
-- ─── Categories ───────────────────────────────────────────────────────────────
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_categories_tenant_active
  ON categories (tenant_id, is_active, sort_order)
  WHERE is_active = TRUE;

-- ─── Payments ─────────────────────────────────────────────────────────────────
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_payments_tenant_created_at
...
-- ─── Stock Movements ──────────────────────────────────────────────────────────
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_stock_movements_product_created_at
  ON stock_movements (product_id, created_at DESC);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_stock_movements_tenant_created_at
  ON stock_movements (tenant_id, created_at DESC);

-- ─── Members ──────────────────────────────────────────────────────────────────
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_members_tenant_phone
  ON members (tenant_id, phone)
  WHERE phone IS NOT NULL;

-- ─── Tables ───────────────────────────────────────────────────────────────────
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_tables_tenant_zone_active
